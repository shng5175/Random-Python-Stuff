...
blueSparki = serial.Serial(str('COM3'),int(9600),write_timeout = 1, timeout = 1)
redSparki = serial.Serial(str('COM5'),int(9600),write_timeout = 1, timeout = 1)
...
##    green_mask = cv2.inRange(rgb, lower_green, upper_green)
    ...
    lower_yellow = np.array([183, 162, 96]) #41,6,10
    upper_yellow = np.array([214, 195, 196]) #90,30,32
...
#First find closest block
    ...
    for i in range(len(blue_path)-1):
      if (blue_path[i+1] != -1):
    	...
    	if theta < 10:
   	     ...
   	     theta = str(theta)
  	print theta
      blueSparki.write('3,'+str(map_index)+','+str(theta)+'n')
      print "Output to blue Sparki"
    ...
    for i in range(len(red_path)-1):
      if (red_path[i+1] != -1):
   	...
   	elif blue_path[i]>9 and red_path[i]<100:
    	...
    	if theta < 10:
   	     ...
   	     theta = str(theta)
  	print theta
	redSparki.write('3,'+str(map_index)+','+str(theta)+'n')
