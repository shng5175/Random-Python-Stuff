import cv2 import numpy as np import serial import math import
matplotlib.pyplot as plt import time
...
while(cap.isOpened()):
  ...
  if ret == True:
    ...
    lower_yellow = np.array([209, 186, 13]) #41,6,10
    upper_yellow = np.array([214, 196, 12]) #90,30,32
    ...
    for i in range(len(blue_path)-1):
        if (blue_path[i+1] != -1):
            ...
            if theta < 10:
                ...
                theta = str(theta)
            print theta
        elif(blue_path[i+1] == -1):
            if blue_has_block == False:
                blue_has_block = True
            elif blue_has_block == True:
                blue_has_block = False
        blueSparki.write('3,'+str(blue_path[i])+','+str(theta)+'n')
        time.sleep(0.5)
